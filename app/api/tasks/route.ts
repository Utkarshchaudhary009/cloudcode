...
import { getGitHubUser, createPullRequest } from '@/lib/github/client'
...
async function processTask(
  ...
  try {
    ...
    if (agentResult.success) {
      ...
      // Auto-create PR if enabled and push was successful
      if (!pushResult.pushFailed && autoCreatePr && branchName && repoUrl) {
        await logger.info('Auto-creating Pull Request...')
        try {
          // Use the prompt as the PR title (truncated) or generate one
          const prTitle = commitMessage.split('\n')[0] // Use the first line of commit message

          const prResult = await createPullRequest({
            repoUrl,
            branchName,
            title: prTitle,
            body: `This PR was automatically created by the Coding Agent task.\n\nTask: ${prompt}\n\nGenerated by [OpenCode](https://opencode.ai)`,
          })

          if (prResult.success) {
            await logger.success(`Pull Request created: ${prResult.prUrl}`)
            ...
            await logger.error(`Failed to auto-create PR: ${prResult.error}`)
          }
        } catch (prError) {
          console.error('Error auto-creating PR:', prError)
          await logger.error('Failed to auto-create PR')
        }
      }
