        ...
        try {
            if (project.isSubscribed) {
                ...
                if (res.ok) {
                    setProjects((prev) =>
                        prev.map((p) =>
                            p.id === project.id ? { ...p, autoFixEnabled: !p.autoFixEnabled } : p,
                        ),
                    )
        ...
        <div className="container py-8 max-w-5xl">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="font-semibold">
                                        {projects.filter((p) => p.autoFixEnabled).length} projects protected
                                    </p>
                                    <p className="text-sm text-muted-foreground">
                                        Build failures will be automatically fixed
                                    </p>
                                </div>
                            </div>
                            <Link href="/vercel-fixes">
            ...
            <div className="grid gap-4 md:grid-cols-2">
                {projects.map((project) => (
                    <Card
                        key={project.id}
                        className={`transition-all ${project.autoFixEnabled
                        ...
                        <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                                {togglingProject === project.id ? (
                                    ...
                                    <Switch
                                        checked={project.autoFixEnabled}
                                        onCheckedChange={() => handleToggleAutoFix(project)}
                                    />
                                )}
                            </div>
                        </CardHeader>
            ...
            {projects.length === 0 && (
                <Card className="border-dashed">
                    <CardContent className="py-12 text-center">
                        ...
                        <p className="text-muted-foreground">
                            Make sure your Vercel API key has access to your projects.
                        </p>
                    </CardContent>
                </Card>
            )}
